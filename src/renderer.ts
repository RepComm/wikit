...
import { addExtensionsToContext, Arrays, BufferInfo, createBufferInfoFromArrays, createVertexArrayInfo, m4, ProgramInfo, resizeCanvasToDisplaySize, setBuffersAndAttributes, setUniforms, v3, VertexArrayInfo } from "twgl.js";
...
export class Renderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    for (let i=0; i<this.quadBrush.max-1; i++) {
      ...
      splat.transform
      .setPos( rand(-50, 50), rand(-50, 50), rand(-50, 50) )
      .setEuler( rand(-Math.PI, Math.PI ), rand(-Math.PI, Math.PI ), rand(-Math.PI, Math.PI ) )
      .setScale( rand(1, 4), rand(1, 4), rand(1, 4));
    }
    setTimeout(()=>{
